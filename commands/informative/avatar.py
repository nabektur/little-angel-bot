...
class Avatar(commands.Cog):
    ...
    async def avatar(self, interaction: discord.Interaction, member: typing.Union[discord.Member, discord.User]=None):
        ...
        embeds.append(discord.Embed(title=f"Аватар {user}", color=member.accent_color or member.color, url=f"https://discord.com/users/{member.id}").set_image(url=f"attachment://{user_avatar.filename}"))
        avatars.append(user_avatar)

        if isinstance(member, discord.Member):
            if member.guild_avatar:
                guild_avatar = await member.display_avatar.to_file()
                embeds.append(discord.Embed(title=f"Аватар {user} на сервере", color=member.accent_color or member.color, url=f"https://discord.com/users/{member.id}").set_image(url=f"attachment://{guild_avatar.filename}"))
